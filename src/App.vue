...
onBeforeMount(async()=>{
  ...
  players.value = await LotteryContract.methods.getPlayers().call();
...
async function pickAWinner(){
   ...
   onPickingAWinner.value = true;
    try{
      ...
      players.value = await LotteryContract.methods.lastWinner().call();  //update lastWinner
      prizePool.value = 0;
    ...
    <div class="flex flex-wrap px-5 mt-10 gap-10">
      <div class="flex-1">
        <div class="bg-white px-5 py-5 rounded">
          <span class="text-3xl font-medium dark:text-white">Lottery Contract</span>
          ...
          <ul class="mt-5 list-disc ml-5">
            <li class="text-gray-900 mb-2"><span class="font-medium">Contract address: </span>{{ contractAddress }}</li>
            <li class="text-gray-900 mb-2"><span class="font-medium">Last Winner: </span> {{ lastWinner ? lastWinner : 'Nothing' }}</li>
            <li class="text-gray-900 mb-2"><span class="font-medium">Manager Address: </span> {{ managerAddres }}</li>
            <li class="text-gray-900 mb-2"><span class="font-medium">Cantidad de Jugadores: {{ cantPlayers }} </span> </li>
          </ul>
        </div>
      </div>
